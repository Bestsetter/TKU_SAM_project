...
class Application(tk.Frame):
    ...
    def create_GUI(self, configfile_path):
        ...
        self.checkbutton_base(left_frame, self.unetpp , self.config["unetpp_text"], background=self.config["background1"][0], foreground = self.config["background1"][2])
    ...
    def gen_ans_img(self):
        t = threading.Thread(target=gen_ans, args=(self.config["oentheSAM.py"],int(self.gen_is_show_ans.get()),int(self.gen_is_gen_compare.get()),int(self.gen_is_show_compare.get()),int(self.unetpp.get())))
